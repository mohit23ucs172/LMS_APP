...
const AddCourse = () => {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} action="" className='flex flex-col gap-4 w-full max-w-md text-gray-500'>
      ...
      <input onChange={e=>setCourseTitle(e.target.value)} value={courseTitle} type="text" placeholder='Type here' className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' required/>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thunbnailImage" className='flex items-center gap-3'>
        ...
        <div>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex items-center justify-between p-4 border-b'>
                <div className='flex items-center'>
                  <img onClick={()=>handleChapter('toggle',chapter.chapterId)}  width={14} src={assets.dropdown_icon} alt="" className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`}/>
              ...
              {chapter.chapterContent.map((lecture,lectureIndex)=>(
                <div key={lectureIndex} className='flex items-center justify-between mb-2'>
                  <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl} target='blank' className='text-blue-500'>Link</a> - {lecture.isPrevieFree ? 'Free Preview' : 'Paid'}</span>
          ...
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center  bg-opacity-50'>
                <div className='bg-white border text-gray-700 p-4 rounded relative w-full max-w-80'>
                    <h2 className='text-lg font-semibold mb-4'>Add Lecture</h2>

                    <div className='mb-2'>
                      <p>Lecture Title</p>
                      <input className='mt-1 block border w-full rounded py-1 px-2' value={lectureDetails.lectureTitle}
                   ...
                   <div className='mb-2'>
                      ...
                      <input className='mt-1 block border w-full rounded py-1 px-2' value={lectureDetails.lectureDuration}
                      ...
                      <div className='mb-2'>
                      <p>Lecture URL</p>
                      <input className='mt-1 block border w-full rounded py-1 px-2' value={lectureDetails.lectureUrl}
                     ...
                     <div className='mb-2'>
                      <p>Is Preview Free?</p>
                      <input className='mt-1 block border w-full rounded py-1 px-2' 
                      onChange={(e=>setLectureDetails({ ...lectureDetails,isPrevieFree:e.target.checked}))}
                       type="checkbox" name="" id="" />
                    </div>
                    
                    <button type='button'
                    onClick={addLecture} className='w-full bg-blue-400 text-white px-4 py-2 rounded'>Add</button>
